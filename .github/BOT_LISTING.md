...
Sentinel helps Discord server administrators protect their communities from moderation mistakes and raid activity. Configure limits for role changes, channel changes, kicks, bans, and unbans, then choose a time window and response. Sentinel can remove roles, kick users, or ban users when a moderator reaches a configured threshold, while keeping a history of actions and triggered limits for review.
...
Sentinel needs access to your server's audit log and the moderation permissions required by the responses you choose. Grant Manage Roles only when using the Remove All Roles response, Kick Members when using Kick User, and Ban Members when using Ban User.
...
- Watch moderation activity, set thresholds, and respond automatically with Sentinel.
